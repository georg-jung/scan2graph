...
func (s *Store) commitReservation(id string, job Job, dir string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.closed {
		return ErrClosed
	...
	delete(s.reservations, id)
	s.jobs[job.ID] = &jobRecord{job: job, dir: dir}
	return nil
}
...
func (s *Store) ReplaceDocument(jobID, docID, newPath string, ocrApplied bool) error {
	...
	if !isWithinDir(rec.dir, newPath) {
		s.mu.Unlock()
		return fmt.Errorf("jobs: replace document: path is outside the job directory")
	}
	idx := -1
...
func (st *Staging) CreateFile(prefix string) (*os.File, error) {
	...
	f, err := os.CreateTemp(st.dir, prefix+"-*")
	if err != nil {
		return nil, fmt.Errorf("jobs: create staged file: %w", err)
	}
	return f, nil
}
...
func (st *Staging) Commit(n NewJob) (Job, error) {
	...
	for _, d := range n.Documents {
		if !isWithinDir(st.dir, d.Path) {
			return Job{}, fmt.Errorf("jobs: document path %q is outside the staging directory", d.Path)
		}
		// Lstat, not Stat: the file must be a regular file that this store
		// created, never a symlink pointing somewhere else. It also means a
		// job can never be committed for a file that is not there.
	...
	if err := st.store.commitReservation(st.id, job, st.dir); err != nil {
		return Job{}, err
	}
...
// isWithinDir reports whether path is strictly inside dir (not dir itself),
// after cleaning both. Used to reject any document path that isn't
// genuinely inside a staging/job directory, so a path never gets built from
